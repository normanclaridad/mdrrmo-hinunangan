...
$urlBack = BASE_URL . '/views/situational-reports/sitrep/view.php?id='. $id;
...
$resSitTrepDetails = $sitrepDetails->getSitrepDetails(' AND sd.id = ' . $id);

if(empty($resSitTrepDetails)) {    
    echo json_encode(['code' => 5, 'message' => 'Invalid request. No record found.']);
    return;
}

$sitrepNo = $resSitTrepDetails[0]['sitrep_no'];
$sitrepName = $resSitTrepDetails[0]['name'] . ' | Sitrep #:' . $sitrepNo;

$sitrepHeaderId = $resSitTrepDetails[0]['sitrep_header_id'];
...
// Suspensions data
$resSuspension = $dbSuspensions->getSuspension(" AND s.sitrep_detail_id = $id AND s.sitrep_header_id = $sitrepHeaderId");
// Landslides
$resLandslides = $dbLandslides->getWhere(" AND sitrep_detail_id = $id AND sitrep_header_id = $sitrepHeaderId");
$landslidesArr = [];
foreach($resLandslides as $land) {
    array_push($landslidesArr, $land['barangay_id']);
}

// Floods
$resFloods = $dbFloods->getWhere(" AND sitrep_detail_id = $id AND sitrep_header_id = $sitrepHeaderId");
$floodsArr = [];
foreach($resFloods as $fl) {
    array_push($floodsArr, $fl['barangay_id']);
}

// Bridges
$resSitrepBridges = $sitrepBridges->getSitrepBridges("AND sb.sitrep_detail_id = $id AND sb.sitrep_header_id = $sitrepHeaderId");
// Roads
$resSitrepRoads = $sitrepRoads->getSitrepRoads("AND sr.sitrep_detail_id = $id AND sr.sitrep_header_id = $sitrepHeaderId");
//Damages
$resDamages = $dbDamages->getDamages("AND d.sitrep_detail_id = $id AND d.sitrep_header_id = $sitrepHeaderId");
//Casualties
$resCasualties = $dbCasualties->getCasualties("AND c.sitrep_detail_id = $id AND c.sitrep_header_id = $sitrepHeaderId");
// Evacuees
$resEvacuees = $dbEvacuees->getEvacuees("AND e.sitrep_detail_id = $id AND e.sitrep_header_id = $sitrepHeaderId");
// WaterLevels
$resWaterLevels = $dbWaterLevels->getWaterLevels("AND wl.sitrep_detail_id = $id AND wl.sitrep_header_id = $sitrepHeaderId");
// Electricity
$resElectricity = $dbElectricity->getElectricity("AND e.sitrep_detail_id = $id AND e.sitrep_header_id = $sitrepHeaderId");
// Stranded
$resStranded = $dbStranded->getWhere("AND sitrep_detail_id = $id AND sitrep_header_id = $sitrepHeaderId");
//Communications
$resCommunications = $dbCommunication->getCommunications("AND c.sitrep_detail_id = $id AND c.sitrep_header_id = $sitrepHeaderId");
//Preparedness Measures
$resPrepMeasures = $dbPreparednessMeasures->getWhere("AND sitrep_detail_id = $id AND sitrep_header_id = $sitrepHeaderId");
        ...
        <div class="content-wrapper">
            ...
            <div class="card">
                <div class="card-body">
                    ...
                    <form id="frm-crud" method="post" data-parsley-validate="">
                        ...
                        <div class="row">
                            <div class="col-md-3">
                                <div class="form-group">
                                    ...
                                    <input type="date" class="form-control" id="date" name="date" value="<?php echo date('Y-m-d', strtotime($resSitTrepDetails[0]['sitrep_datetime'])) ?>" placeholder="" required>
                                </div>
                            </div>                                
                            <div class="col-md-3">
                                <div class="form-group">
                                    <label for="time">Time</label>
                                    <input type="time" class="form-control" id="time" name="time" value="<?php echo date('H:i:s', strtotime($resSitTrepDetails[0]['sitrep_datetime'])) ?>" placeholder="" required>
                                </div>
                            </div>
                        </div>
                        <fieldset>
                            ...
                            <div class="">
                                <table id="schools" class="table table-responsive">
                                    ...
                                    <tbody>
                                        <?php foreach($resSuspension AS $suspen): ?>
                                            <tr data-id="<?php echo $suspen['id'] ?>">
                                                <td style="display: none">
                                                    <input type="hidden" class="school-id" name="school_id[]" value="<?php echo $suspen['id'] ?>" />
                                                </td>
                                                <td><?php echo $suspen['school_name'] ?></td>
                                                <td><?php echo $suspen['brgyDesc'] ?></td>
                                                <td class="text-center"><icon role="button" onclick="removeSchool(this)">x</icon></td>
                                            </tr>
                                        <?php endforeach; ?>
                                    </tbody>
                                </table>
                                    ...
                                    <div class="form-group">
                                        ...
                                        <select class="form-control" id="landslide_barangay" name="landslide_barangay[]" multiple style="width: 100%;">
                                            <?php foreach($resBarangays as $barangay):
                                                $selected = (in_array($barangay['id'], $landslidesArr)) ? 'selected' : '';
                                                ?>
                                                <option value="<?php echo $barangay['id'] ?>" <?php echo $selected ?>>
                                ...
                                <div class="col-md-10">
                                    <div class="form-group">
                                        ...
                                        <select class="form-control" id="flood_barangay" name="flood_barangay[]" multiple style="width: 100%;">
                                            <?php foreach($resBarangays as $barangay): 
                                                $selected = (in_array($barangay['id'], $floodsArr)) ? 'selected' : '';
                                                ?>
                                                <option value="<?php echo $barangay['id'] ?>" <?php echo $selected ?>>
                                                    <?php echo $barangay['brgyDesc'] ?>
                                                </option>
                                ...
                                <div class="col-md-10">
                                    <div>
                                        ...
                                        <table id="bridges" class="table table-grid table-responsive">
                                            ...
                                            <tbody>
                                                <?php foreach($resSitrepBridges as $bri): ?>
                                                    <tr>
                                                        <td>
                                                            <select  id="bridge" name="bridge[]">
                                                                <option value="">Select</option>
                                                                <?php foreach($resBridges as $bridge): 
                                                                    $select = ($bri['bridge_id'] == $bridge['id']) ? 'selected' : '';
                                                                ?>
                                                                <option value="<?php echo $bridge['id'] ?>" data-brgy-name="<?php echo $bridge['brgyDesc'] ?>" <?php echo $select ?>>
                                                                    <?php echo $bridge['name'] ?>
                                                                </option>
                                                                <?php endforeach; ?>
                                                            </select>
                                                        </td>
                                                        <td><?php echo $bri['bridge_name'] ?></td>
                                                        <td>
                                                            <input type="text" class="remarks[]" value="<?php echo $bri['remarks'] ?>" />
                                                        </td>
                                                        <td class="text-center"><icon role="button" onclick="removeSchool(this)">x</icon></td>
                                                    </tr>
                                                <?php endforeach; ?>
                                ...
                                <div class="col-md-10">
                                    <div>
                                        ...
                                        <table id="roads" class="table table-grid table-responsive">
                                            ...
                                            <tbody>
                                            <?php foreach($resSitrepRoads as $ro): ?>
                                                    <tr>
                                                        <td>
                                                            <select  id="road" name="bridge[]">
                                                                <option value="">Select</option>
                                                                <?php foreach($resRoads as $road): 
                                                                    $select = ($road['id'] == $ro['road_id']) ? 'selected' : '';
                                                                ?>
                                                                <option value="<?php echo $road['id'] ?>" <?php echo $select ?>>
                                                                    <?php echo $road['name'] ?>
                                                                </option>
                                                                <?php endforeach; ?>
                                                            </select>
                                                        ...
                                                        <td>
                                                            <input type="text" class="remarks[]" value="<?php echo $ro['remarks'] ?>" />
                                                        </td>
                                                        <td class="text-center"><icon role="button" onclick="removeSchool(this)">x</icon></td>
                                                    </tr>
                                                <?php endforeach; ?>                   
                                ...
                                <div class="col-md-10">
                                    <div>
                                        ...
                                        <table id="damages" class="table table-grid table-responsive">
                                            ...
                                            <tbody>
                                                <?php foreach($resDamages as $dam): ?>                       
                                                    <tr>
                                                        <td>
                                                            <select  id="damages_barangay" name="damages_barangay[]">
                                                                <option value="">Select</option>
                                                                <?php foreach($resBarangays as $barangay):
                                                                    $select = ($dam['baragay_id'] == $barangay['id']) ? 'selected' : '';
                                                                    ?>
                                                                    <option value="<?php echo $barangay['id'] ?>" <?php echo $select ?>>
                                                                        <?php echo $barangay['brgyDesc'] ?>
                                                                    </option>
                                                                <?php endforeach; ?>
                                                            </select>
                                                        ...
                                                        <td>
                                                            <select  id="facility_types" name="facility_types[]">
                                                                <option value="">Select</option>
                                                                <?php foreach($resFacilityTypes as $faciType): 
                                                                    $select = ($dam['facility_type_id'] == $faciType['id']) ? 'selected' : '';    
                                                                ?>
                                                                    <option value="<?php echo $faciType['id'] ?>" <?php echo $select ?>>
                                                                        <?php echo $faciType['name'] ?>
                                                                    </option>
                                                                <?php endforeach; ?>
                                                            </select>
                                                        </td>
                                                        <td>
                                                            <input type="number" class="damage_count" value="<?php echo $dam['damage_count']  ?>" />
                                                        </td>
                                                        <td>
                                                            <select  id="damage_type" name="damage_type[]">
                                                                <option value="">Select</option>
                                                                <option value="P" <?php echo ($dam['damage_type'] == 'P') ? 'selected' : '' ?>>Partially</option>
                                                                <option value="T" <?php echo ($dam['damage_type'] == 'T') ? 'selected' : '' ?>>Totally</option>
                                                            </select>
                                                        </td>
                                                        <td class="text-center"><icon role="button" onclick="removeSchool(this)">x</icon></td>
                                                    </tr>
                                ...
                                <div class="col-md-10">
                                    <div>
                                        ...
                                        <table id="casualties" class="table table-grid table-responsive">
                                            ...
                                            <tbody>
                                                <?php foreach($resCasualties as $cas): ?>
                                                    <tr>
                                                        <td>
                                                            <select id="casualties_barangay" name="casualties_barangay">
                                                                <option value="">Select</option>
                                                                <?php foreach($resBarangays as $barangay):
                                                                    $select = ($barangay['id'] == $cas['baragay_id']) ? 'selected' : '';
                                                                    ?>
                                                                    <option value="<?php echo $barangay['id'] ?>" <?php echo $select ?>>
                                                                        <?php echo $barangay['brgyDesc'] ?>
                                                                    </option>
                                                        ...
                                                        <td>
                                                            <select  id="moi" name="moi">
                                                                ...
                                                                <?php foreach($resMechanismOfInjuries as $moi):
                                                                    $select = ($moi['id'] == $cas['moi_id']) ? 'selected' : '';                                                                    
                                                                    ?>
                                                                    <option value="<?php echo $moi['id'] ?>" <?php echo $select ?>>
                                                                        <?php echo $moi['name'] ?>
                                                                    </option>
                                                                    <?php endforeach; ?>
                                                                </select>
                                                            ...
                                                            <td>
                                                                <select id="age_bracket" name="age_bracket">
                                                                    ...
                                                                    <?php foreach($ageBrackets as $ageBracket):
                                                                     $select = ($ageBracket['id'] == $cas['age_bracket_id']) ? 'selected' : '';                                                                    
                                                                     ?>
                                                                    <option value="<?php echo $ageBracket['id'] ?>" <?php echo $select ?>>
                                                                        <?php echo $ageBracket['age_from'] . ' - ' . $ageBracket['age_to'] ?>
                                                                    </option>
                                                                <?php endforeach; ?>
                                                            </select>
                                                        </td>
                                                        <td>
                                                            <select id="gender" name="gender[]">
                                                                <option value="">Select</option>
                                                                <option value="M" <?php echo ($cas['gender'] == 'M') ? 'selected' : '' ?>>Male</option>
                                                                <option value="F" <?php echo ($cas['gender'] == 'F') ? 'selected' : '' ?>>Female</option>
                                                                <option value="L" <?php echo ($cas['gender'] == 'L') ? 'selected' : '' ?>>LGBTQIA+</option>
                                                            </select>
                                                        </td>
                                                        <td>
                                                            <input type="number" class="casualty_count" value="<?php echo $cas['count'] ?>" style="width: 40px" />
                                                        </td>
                                                        <td class="text-center"><icon role="button" onclick="removeSchool(this)">x</icon></td>
                                                    </tr>
                                                <?php endforeach; ?>
                                ...
                                <div class="col-md-10">
                                    <div>
                                        ...
                                        <table id="evacuees" class="table table-grid table-responsive">
                                            ...
                                            <tbody>
                                                <?php foreach($resEvacuees as $evac): ?>
                                                    <tr>
                                                        <td>
                                                            <select id="evacuees_barangay" name="evacuees_barangay">
                                                                <option value="">Select</option>
                                                                <?php foreach($resBarangays as $barangay):
                                                                    $select = ($barangay['id'] == $evac['baragay_id']) ? 'selected' : '';
                                                                    ?>
                                                                    <option value="<?php echo $barangay['id'] ?>" <?php echo $select ?>>
                                                                        <?php echo $barangay['brgyDesc'] ?>
                                                                    </option>
                                                        ...
                                                        <td>
                                                            <select id="age_bracket" name="age_bracket">
                                                                <option value="">Select</option>
                                                                <?php foreach($ageBrackets as $ageBracket):
                                                                $select = ($ageBracket['id'] == $evac['age_bracket_id']) ? 'selected' : '';                                                                    
                                                                ?>
                                                                <option value="<?php echo $ageBracket['id'] ?>" <?php echo $select ?>>
                                                                    <?php echo $ageBracket['age_from'] . ' - ' . $ageBracket['age_to'] ?>
                                                                </option>
                                                        ...
                                                        <td>
                                                            <select id="gender" name="gender[]">
                                                                <option value="">Select</option>
                                                                <option value="M" <?php echo ($evac['gender'] == 'M') ? 'selected' : '' ?>>Male</option>
                                                                <option value="F" <?php echo ($evac['gender'] == 'F') ? 'selected' : '' ?>>Female</option>
                                                                <option value="L" <?php echo ($evac['gender'] == 'L') ? 'selected' : '' ?>>LGBTQIA+</option>
                                                            </select>
                                                        </td>
                                                        <td>
                                                            <input type="number" class="evacuees_count" value="<?php echo $evac['count'] ?>" style="width: 50px;" />
                                                        </td>
                                                        <td class="text-center"><icon role="button" onclick="removeSchool(this)">x</icon></td>
                                                    </tr>
                                                <?php endforeach; ?>
                                ...
                                <div class="col-md-10">
                                    <div>
                                        ...
                                        <table id="water_levels" class="table table-grid table-responsive">
                                            ...
                                            <tbody>
                                                <?php foreach($resWaterLevels as $wl): ?>
                                                    <tr>
                                                        <td>
                                                            <select id="water_level_barangay" name="water_level_barangay">
                                                                <option value="">Select</option>
                                                                <?php foreach($resBarangays as $barangay):
                                                                    $select = ($barangay['id'] == $wl['baragay_id']) ? 'selected' : '';
                                                                    ?>
                                                                    <option value="<?php echo $barangay['id'] ?>" <?php echo $select ?>>
                                                                        <?php echo $barangay['brgyDesc'] ?>
                                                                    </option>
                                                        ...
                                                        <td>
                                                            <input type="number" class="water_level_count" value="<?php echo $wl['level'] ?>" style="width: 50px;" />
                                                        </td>
                                                        <td class="text-center"><icon role="button" onclick="removeSchool(this)">x</icon></td>
                                                    </tr>
                                                <?php endforeach; ?>
                                ...
                                <div class="col-md-10">
                                    <div>
                                        ...
                                        <table id="electricity" class="table table-grid table-responsive">
                                            ...
                                            <tbody>
                                                <?php foreach($resElectricity as $el) : ?>
                                                    <tr>
                                                        <td>
                                                            <select id="electricity_barangay" name="electricity_barangay">
                                                                <option value="">Select</option>
                                                                <?php foreach($resBarangays as $barangay):
                                                                    $select = ($barangay['id'] == $el['baragay_id']) ? 'selected' : '';
                                                                    ?>
                                                                    <option value="<?php echo $barangay['id'] ?>" <?php echo $select ?>>
                                                                        <?php echo $barangay['brgyDesc'] ?>
                                                                    </option>
                                                        ...
                                                        <td>
                                                            <select id="electricity_status" name="electricity_status">
                                                                <option value="">Select</option>
                                                                <option value="O" <?php echo ($el['status'] == 'O') ? 'selected' : '' ?>>Power Outage</option>
                                                                <option value="R" <?php echo ($el['status'] == 'R') ? 'selected' : '' ?>>Restored</option>
                                                            </select>
                                                        </td>
                                                        <td>
                                                            <input type="date" class="status_date" style="width: 110px;" value="<?php echo date('Y-m-d', strtotime($el['date'])) ?>" />
                                                            <input type="time" class="status_time" style="width: 70px;" value="<?php echo $el['time'] ?>"/>
                                                        </td>
                                                        <td class="text-center"><icon role="button" onclick="removeSchool(this)">x</icon></td>
                                                    </tr>
                                                <?php endforeach; ?>                  
                                ...
                                <div class="col-md-10">
                                    <div>
                                        ...
                                        <table id="stranded" class="table table-grid table-responsive">
                                            ...
                                            <tbody>
                                                <?php foreach($resStranded as $strand): ?>
                                                    <tr>
                                                        <td>
                                                            <select id="stranded_location" name="stranded_location">
                                                                <option value="">Select</option>
                                                                <option value="T" <?php echo ($strand['location'] == 'T') ? 'selected' : '' ?>>Bus Terminal</option>
                                                                <option value="W" <?php echo ($strand['location'] == 'W') ? 'selected' : '' ?>>WHARF</option>
                                                            </select>
                                                        </td>
                                                        <td>
                                                            <select  id="stranded_type" name="stranded_type">
                                                                <option value="">Select</option>
                                                                <option value="T" <?php echo ($strand['type'] == 'T') ? 'selected' : '' ?>>Tourist</option>
                                                                <option value="L" <?php echo ($strand['type'] == 'L') ? 'selected' : '' ?>>Local</option>
                                                            </select>
                                                        </td>
                                                        <td>
                                                            <input type="number" class="stranded_count" value="<?php echo $strand['count'] ?>" style="width: 50px;" />
                                                        </td>
                                                        <td class="text-center"><icon role="button" onclick="removeSchool(this)">x</icon></td>
                                                    </tr>
                                                <?php endforeach; ?>
                                ...
                                <div class="col-md-10">
                                    <div>
                                        ...
                                        <table id="communications" class="table table-grid table-responsive">
                                            ...
                                            <tbody>
                                                <?php foreach($resCommunications as $com): ?>    
                                                    <tr>
                                                        <td>
                                                            <select id="telco" name="telco">
                                                                <option value="">Select</option>
                                                                <?php foreach($resTelco as $tel):
                                                                    $select = ($tel['id'] == $com['telco_id']) ? 'selected' : '';
                                                                    ?>
                                                                    <option value="<?php echo $tel['id'] ?>" <?php echo $select ?>>
                                                                        <?php echo $tel['name'] ?>
                                                                    </option>
                                                                <?php endforeach; ?>
                                                            </select>
                                                        ...
                                                        <td>
                                                            <select id="status" name="status">
                                                                <option value="">Select</option>
                                                                <option value="N" <?php echo ($com['status'] == 'N') ? 'selected' : '' ?>>No Service</option>
                                                                <option value="R" <?php echo ($com['status'] == 'R') ? 'selected' : '' ?>>Restored</option>
                                                            </select>
                                                        </td>
                                                        <td>
                                                            <input type="date" class="telco_status_date" style="width: 110px" value="<?php echo date("Y-m-d", strtotime($com['date'])) ?>" />
                                                            <input type="time" class="telco_status_time" style="width: 70px" value="<?php echo $com['time'] ?>"/>
                                                        </td>
                                                        <td class="text-center"><icon role="button" onclick="removeSchool(this)">x</icon></td>
                                                    </tr>
                                                <?php endforeach; ?>    
                        ...
                        <fieldset>
                            ...
                            <div class="row">                                    
                                <div class="col-md-12">
                                    <textarea id="preparedness_measures" name="preparedness_measures" class="form-control"><?php echo !empty($resPrepMeasures) ? $resPrepMeasures[0]['content'] : '' ?></textarea>
                                </div>
                            </div>
                        </fieldset>
                    </form>
                ...
                <div class="card-footer">
                    <a href="<?php echo $urlBack ?>" class="btn btn-secondary">Back</a>
...
<script>
    $(document).ready(function(){
        ...
        $('#btn-save').click(function(){
            ...
            var data = {
                id : <?php echo $id ?>,
                setripHeaderId : <?php echo $sitrepHeaderId ?>,
