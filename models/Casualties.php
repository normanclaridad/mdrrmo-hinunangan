...
class Casualties extends Models {
    ...
    public function getCasualties( $where = '', $sortBy = 'c.id ASC') {
		$sql = "SELECT c.*, b.brgyDesc, 
                m.name AS moi_name, ab.age_from, ab.age_to
                FROM casualties c
                JOIN barangays b ON b.id = c.baragay_id
                JOIN mechanism_of_injuries m ON m.id = c.moi_id
                JOIN age_brackets ab ON ab.id = c.age_bracket_id
                WHERE 1";
