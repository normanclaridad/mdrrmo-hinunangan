...
class Damages extends Models {
    ...
    public function getDamages( $where = '', $sortBy = 'd.id ASC') {
		$sql = "SELECT d.*, b.brgyDesc, ft.name AS facility_id_name 
                FROM damages d
                JOIN barangays b ON b.id = d.baragay_id
                JOIN facility_types ft ON ft.id = d.facility_type_id
                WHERE 1 ";
