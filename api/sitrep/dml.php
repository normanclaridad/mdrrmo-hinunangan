...
$sitrepHeaders = new Sitrep_headers();
...
if(in_array($actionType, ['add', 'update'])) {    
    ...
    $checkModule = $sitrepHeaders->getWhere($where);
...
//Add data
if($actionType == 'add') {
    $resResult = $sitrepHeaders->insertData($data);
} else {
    $where = " id = $id";
    $resResult = $sitrepHeaders->updateData($data, $where);
}

if(!$resResult){
    echo json_dencode(['code' => 1, 'message' => 'Internal error. Please contact administrator.']);
    return;
}
