...
//Get total
$totalRecords = $waterLevelAlarm->getTotal($whereCondition, $sortBy);

//Get all tsag
$resResults = $waterLevelAlarm->getJoinWhere($whereCondition, $sortBy, $start, $length);
$data = [];
foreach($resResults AS $row) {
    $encryptedId = $helpers->encryptDecrypt($row['id']);
    $action = '<a class="btn btn-sm btn-edit" data-id="'. $row['id'] .'" data-level-from="'. $row['level_from'] .'"data-level-to="'. $row['level_to'] . '"data-alarm-level="'. $row['alarm_level'] . '"  data-status="' . $row['status'] . '"><i class="fa fa-edit"></i></a>';
    $action .= '&nbsp; <a class="btn btn-sm btn-delete" data-id="'. $row['id'] .'" data-level-from="'. $row['level_from'] . '"data-level-to="'. $row['level_to'] . '"data-alarm-level="'. $row['alarm_level'] . '" data-status="' . $row['status'] . '"><i class="fa fa-trash"></i></a>';
