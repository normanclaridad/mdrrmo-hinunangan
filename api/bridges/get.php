...
//Get total
$totalRecords = $bridges->getTotal($whereCondition, $sortBy);

//Get all tsag
$resResults = $bridges->getJoinWhere($whereCondition, $sortBy, $start, $length);
$data = [];
foreach($resResults AS $row) {
    ...
    $action = '<a class="btn btn-sm btn-edit" data-id="'. $row['id'] .'" data-name="'. $row['name'] .'" data-barangay-id="'. $row['barangay_id'] .'" data-address="' . $addressName .'" data-status="' . $row['status'] . '"><i class="fa fa-edit"></i></a>';
